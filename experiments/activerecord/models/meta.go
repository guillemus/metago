...
func (q *query[T, PK, Q]) First(ctx context.Context) (*T, error) {
	...
	if len(state.order) == 0 {
		state = state.withOrder(q.model.primaryColumn + " ASC")
	}
	return q.first(ctx, state)
}
...
func (q *query[T, PK, Q]) Exists(ctx context.Context) (bool, error) {
	...
	state := q.state.first()
	var one int
	err := q.db.QueryRowContext(ctx, "SELECT 1 FROM "+q.model.table+where+state.orderSQL()+state.limitSQL(), args...).Scan(&one)
...
func newUserQuery(db DBTX, state queryState) *UserQuery {
	...
	q.WhereScore = newOrderedField[*UserQuery, float64](q.addWhere, "score")
	q.OrderByScore = newOrderField(q.addOrder, "score")
	q.WhereBio = newNullableTextField[*UserQuery, string](q.addWhere, "bio")
	q.WhereRank = newNullableOrderedField[*UserQuery, int](q.addWhere, "rank")
	q.OrderByRank = newOrderField(q.addOrder, "rank")
	return q
}
...
func newWidgetQuery(db DBTX, state queryState) *WidgetQuery {
	...
	q.WhereID = newOrderedField[*WidgetQuery, WidgetID](q.addWhere, "id")
	q.OrderByID = newOrderField(q.addOrder, "id")
	q.WhereLabel = newTextField[*WidgetQuery, string](q.addWhere, "display_label")
	q.OrderByLabel = newOrderField(q.addOrder, "display_label")
	return q
}
...
func newAuditLogQuery(db DBTX, state queryState) *AuditLogQuery {
	...
	q.WhereID = newOrderedField[*AuditLogQuery, AuditLogID](q.addWhere, "id")
	q.OrderByID = newOrderField(q.addOrder, "id")
	q.WhereMessage = newTextField[*AuditLogQuery, string](q.addWhere, "message")
	return q
}
