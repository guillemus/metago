...
Run `metago .` to generate `meta.go` beside the source:
...
Metago is silent on success unless verbose logging is enabled. It skips `vendor`, `testdata`, and
hidden directories. Template names come from `{{ define "name" }}` blocks and must be unique across
the scan root. Names beginning with `std.` are reserved for Metago's standard templates.
