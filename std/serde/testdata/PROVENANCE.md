...
complete JSON text, the four allowed whitespace bytes, structural separators, literals, numbers, and
strings. The fixtures paraphrase or adapt behaviors and do not reproduce RFC prose.
...
moved into the `float64` fixture field, root arrays are moved into the `interface` field, and
invalid byte sequences are represented losslessly as hexadecimal test bytes. `i_number_huge_exp`
uses a shorter exponent that preserves the original overflow behavior. All other numeric tokens and
string payload bytes are retained. Policies follow `encoding/json` and are checked against it in the
test.
